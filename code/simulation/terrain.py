...
def _add_static_box(
    half, pos, orn=(0, 0, 0, 1), color=(0.6, 0.6, 0.6, 1.0), friction=1.2
):
...
def add_ramp(
    y_start, length, rise, width=0.60, friction=1.3, color=(0.35, 0.65, 0.90, 1.0)
):
...
def add_box_obstacle(
    y_center, height, length, width=0.60, friction=1.3, color=(0.9, 0.55, 0.25, 1.0)
):
    ...
    return _add_static_box(
        half, (0.0, y_center, height / 2.0), (0, 0, 0, 1), color, friction
    )
...
def build_terrain_course():
    ...
    ids.append(
        add_box_obstacle(y_center=0.40, height=0.02, length=0.20, color=ramp_color)
    )
    ...
    for y in (0.85, 0.95, 1.05):
        ids.append(
            add_box_obstacle(y_center=y, height=0.006, length=0.010, color=bump_color)
        )

    ids.append(
        add_box_obstacle(
            y_center=1.35, height=0.010, length=0.25, color=(0.85, 0.35, 0.30, 1.0)
        )
    )
...
def run_terrain(gait_name, gui=True, duration=40.0, return_report=False):
    ...
    print("  course: " + " -> ".join(cp["name"] for cp in COURSE))
    ...
    for i in range(steps):
        ...
        if fell:
            for cp in COURSE:
                if reached[cp["name"]] is None:
                    ...
                    break
            print(
                f"  [t={t:5.2f}s] FELL before {fell_at} "
        ...
        for cp in COURSE:
            if reached[cp["name"]] is None and pos[1] >= cp["y"]:
                ...
                print(
                    f"  [t={t:5.2f}s] {cp['name']:10s} y={pos[1]:+.2f} "
...
def run_terrain_matrix(gui=False, save_path=None):
    ...
    for gait in ("walk", "trot", "bound"):
        summary = run_terrain(
            gait, gui=gui, duration=durations[gait], return_report=True
        )
        results.append(summary)

    header = (
        ["Gait"]
        + [cp["name"] for cp in COURSE]
