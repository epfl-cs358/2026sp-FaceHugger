...
let _i = 0;
let _timer = null;
const ws = new WebSocket("ws://192.168.4.2:81");
...
function fhStop() {
  ...
  console.log(
    "FaceHugger: playback stopped (servos hold last commanded pose).",
  );
}
...
ws.onopen = () => {
  console.log(
    "FaceHugger: connected — playing " +
      CLIP_NAME +
      " (" +
      CLIP.length +
      " frames). Hold-at-end is on by default; call fhStop() when done.",
  );
  _timer = setInterval(playFrame, FRAME_MS);
};
ws.onerror = (e) => {
  console.error(
    "FaceHugger: WebSocket error. On the robot Wi-Fi (FaceHugger_Net, ws://192.168.4.1:81)? Is this page http:// (NOT https:// — ws:// is blocked from https pages)?",
    e,
  );
};
ws.onclose = () => {
  fhStop();
  console.log("FaceHugger: socket closed.");
};
