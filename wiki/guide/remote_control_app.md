...
=== "Option A — Expo Go (no build required)"
    ...
    The app loads on your phone and hot-reloads on every source change — useful while developing.
...
The ESP32 creates its own Wi-Fi access point — your phone must join that network, not your home Wi-Fi.
...
Direct per-servo angle control, used for calibration. Switching to this tab sends `STATE_IDLE` — the gait engine turns off and servos hold position.
...
Each tap sends a single calibration packet to that servo. The input field remembers the last-sent angle for each leg/servo pair within the session — switching between servos restores the last value you committed.
...
| App connects but robot doesn't respond to joystick | Robot is still in `STATE_IDLE` | Swipe to the Remote Control tab — this triggers `STATE_WALK` |
| Robot stops moving after ~2 seconds of holding still | Firmware 2-second idle timeout — this is intentional | Keep dragging the joystick to sustain movement |
| Invert button does nothing | Battery voltage below the action gate threshold | Charge or replace the battery |
| Telemetry bar stays hidden after connecting | `T:10` status packets not arriving | Check the serial monitor — the firmware may have crashed; power-cycle the robot |
