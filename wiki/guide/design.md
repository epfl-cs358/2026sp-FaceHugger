<!-- Sections below are suggestions, not requirements - keep what's useful, drop or merge the rest. -->
# Design

!!! todo "Stub - to be written"
    How the robot is designed and why, written for a newcomer. The *why* before
    any code or math.

## The concept

!!! todo
    What kind of robot this is (4 legs, 3 joints each) and what it's meant to do.

## How a leg is built

!!! todo
    Three links per leg, the joints, and the range of motion.

## How a leg moves

!!! todo
    Plain-language intuition: which joint moves the foot where; forward vs
    inverse kinematics; the zero pose and sign conventions at a glance. Full
    treatment in [Reference → Kinematics](../reference/firmware/kinematics.md)
    and [servo conventions](../reference/firmware/servo-conventions.md).

## Sizing & loads

!!! todo
    Why the links and servos are sized the way they are; how much weight a leg
    holds. Full torque model in
    [Reference → Torque analysis](../reference/simulation/torque-analysis.md).
