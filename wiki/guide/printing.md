...
!!! todo
    Describe orientation for each link. The `.3mf` files already encode the correct orientation — open them in PrusaSlicer and import as-is.
...
!!! todo
    Describe what each `.3mf` contains and which one to use. `FHLegsPrintThreePlates.3mf` likely has the full set — confirm and add a note.
