...
3. **Wait for the heartbeat** - once the WebSocket is open, the robot begins sending a status packet every 100 ms (10 Hz). The app shows the robot as connected when this arrives.
...
- **Telemetry (robot → app):** `T:10`. The robot sends this every 100 ms (10 Hz) as a heartbeat; the app uses it to display the current state, sensor readings, and any errors.
