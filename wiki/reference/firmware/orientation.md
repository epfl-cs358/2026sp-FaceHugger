...
A user might want auto-flip *off*, for instance when calibrating the robot on its back without the firmware constantly trying to mirror the pose. T:6 (`CMD_SET_AUTO_INVERT`) does exactly that:
...
The header `RobotStatus.tsx` tile displays the live orientation as `Pitch: +12.3° | Roll: -5.7° | Upright`. The formatters live in `code/remote-control-app/MyApp/api/orientation.ts` and render `-` when the field is null, which is important for older firmware that predates the IMU fields so the tile stays graceful instead of crashing on `undefined`.
