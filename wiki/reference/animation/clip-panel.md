...
- **`_frame_to_servo`** is the host twin of the firmware `translateToServo` (lives in `code/simulation/animation_tools/exporter_parity.py`, not in the exporter's `servo_math.py` — the exporter is CALIB-free). Three steps: scale toward `NEUTRAL` by 2/3, apply the per-leg servo branch (`FL: 90 + (sh - 135)`, `FR: 90 + (sh - 45)`, `BL: 90 + (sh + 135)`, `BR: 90 + (sh + 45)`, with thigh/knee mirrored per side), then clamp to 0-180 and round. It powers two surfaces: the firmware-parity test (`gen_clip_parity_reference.py`) and the Blender panel's "current servo angles" preview.
